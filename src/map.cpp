                    ...
                    const unsigned char* map, u32 map_width, u32 map_height, u32 layer)
{
	...
	for (iy = 0; iy < 48; iy++)
	{
		for (ix = 0; ix < 80; ix++)
		{
			...
			m_ls->mapData[bgnr - 1][(iy * 80) + ix].visibleTileIndex = map[(m_ls->levelNum * 3840) + ((
			        iy * 80) + ix)];
...
void CMap::ResetMap()
{
	...
	{
		m_ls->mapData[1][((m_ls->checkpoint.checkpointPos.y) * 80) +
		                 m_ls->checkpoint.checkpointPos.x].tileIndex = 0;
		m_ls->mapChanges[m_ls->mapChangeCount].layer = 1;
...
void CMap::LoadContainers()
{
	...
	for (y = 0; y < 48 ; y++)
	{
		for (x = 0; x < 80 ; x++)
		{
			if ((m_ls->mapData[1][(y * 80) + x].tileIndex == TILE_CHEST)
			    || (m_ls->mapData[1][(y * 80) + x].tileIndex == TILE_URN_TOP))
			{
				...
				if (contentsDataPos < (sizeof(map_containers) / 2) - 1)
				{
					contentsDataPos++;
				}
...
int LookupCheckpoint(T_LEVELSTATE *ls, int x, int y)
{
	...
	while (checkpointDataPos < checkpointDataEnd)
	{
		    ...
		    (y == map_checkpoints[checkpointDataPos + 2]))
		{
			...
			ls->checkpoint.playerStartPos.y = map_checkpoints[checkpointDataPos + 4] -
			                                  1; //Player location is at the tile above
			//Go to bows value
			checkpointDataPos += 5;
