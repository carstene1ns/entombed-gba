			...
			if (newTilePos.y != tilePos.y)
			{
				...
				if ((m_ls->mapData[0][(newTilePos.y * 80) + tilePos.x].tileIndex > 0)
				    || ((newTilePos.y > tilePos.y)
				        && ((m_ls->mapData[1][(newTilePos.y * 80) + tilePos.x].tileIndex == TILE_PLATFORM1) ||
         ...
         int _weight)
{
	...
	if (projectiles.size() == MAX_PROJECTILES)
	{
		...
		projectiles.begin()->m_ls->projectileOamIsFree[projectiles.begin()->oam_index -
		           PROJECTILE_FIRST_OAM] = true;
