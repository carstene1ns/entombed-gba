...
void CPlayer::player_input()
{
	...
	else if( key_is_down(KEY_UP) && (m_state != PLAYER_STATE_FIRING) )
	{
	    ...
	    if ( (m_touchingLadder == true) && ((g_cheatEnabled[2] == false) || (g_cheatEnabled[2] && key_is_down(KEY_B) == false))  )
	    {
...
void CPlayer::player_move()
{
	...
	if (m_state != PLAYER_STATE_DYING)
	{
        ...
        if ((m_lookingDown == false) && (m_lookDownCounter > 0)  && (((m_ls->playerPos.y>>8) - m_ls->vp.y) < 80))
        {
        	m_ls->mapOffset.y -= LOOK_DOWN_SPEED;
        	m_lookDownCounter--;
        }
...
void CPlayer::player_ani_fireArrow()
{
	...
	if (frame == 4)
	{
		if (m_dir == LOOK_LEFT)
		{
			add_projectile(m_ls, ((m_ls->playerPos.x>>8) - m_ls->vp.x) - 8, ((m_ls->playerPos.y>>8) - m_ls->vp.y) + 20,-ARROW_SPEED,0,0,-1,1);
		}
		else //RIGHT
		{
			add_projectile(m_ls, ((m_ls->playerPos.x>>8) - m_ls->vp.x) + 24, ((m_ls->playerPos.y>>8) - m_ls->vp.y) + 20,ARROW_SPEED,0,1,-1,1);
		}
		player_set_state(PLAYER_STATE_STAND);
	}
}
...
void CPlayer::player_test_collisions()
{
    ...
    collisionPoint[0] = ((((m_ls->playerPos.y>>8) + 8 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_LEFT - 1)) >> 5);
    collisionPoint[1] = ((((m_ls->playerPos.y>>8) + 16 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_LEFT - 1)) >> 5);
    collisionPoint[2] = ((((m_ls->playerPos.y>>8) + 31 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_LEFT - 1)) >> 5);
    ...
    if ((m_ls->mapData[1][collisionPoint[0]].tileIndex == TILE_WOOD_FRAME) || (m_ls->mapData[1][collisionPoint[0]].tileIndex == TILE_WOOD_FRAME_TOP) ||
    	(m_ls->mapData[1][collisionPoint[1]].tileIndex == TILE_WOOD_FRAME)	|| (m_ls->mapData[1][collisionPoint[1]].tileIndex == TILE_WOOD_FRAME_TOP) ||
		(m_ls->mapData[1][collisionPoint[2]].tileIndex == TILE_WOOD_FRAME) || (m_ls->mapData[1][collisionPoint[2]].tileIndex == TILE_WOOD_FRAME_TOP))
    {
        m_blockedDirs[0] = 1;
    }
    ...
    collisionPoint[0] = ((((m_ls->playerPos.y>>8) + 8 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_RIGHT + 1)) >> 5);
    collisionPoint[1] = ((((m_ls->playerPos.y>>8) + 16 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_RIGHT + 1)) >> 5);
    collisionPoint[2] = ((((m_ls->playerPos.y>>8) + 31 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_RIGHT + 1)) >> 5);
	...
	collisionPoint[0] = ((((m_ls->playerPos.y>>8) + 7 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_LEFT)) >> 5);
	collisionPoint[1] = ((((m_ls->playerPos.y>>8) + 7 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_RIGHT)) >> 5);

    //Check that we're just below the block boundary (y + 8 % 16 == 0 )
    if (((m_ls->playerPos.y>>8) + 8) % 16 == 0)
    {
    	//Check for wall tiles
        if ((m_ls->mapData[0][collisionPoint[0]].tileIndex > 0) || (m_ls->mapData[0][collisionPoint[1]].tileIndex > 0)  )
        {
            m_blockedDirs[2] = 1;
        }
        //Check for layer 1 blocking tiles (Wooden frame)
        if ((m_ls->mapData[1][collisionPoint[0]].tileIndex == TILE_WOOD_FRAME) || (m_ls->mapData[1][collisionPoint[1]].tileIndex == TILE_WOOD_FRAME) ||
        	(m_ls->mapData[1][collisionPoint[0]].tileIndex == TILE_WOOD_FRAME_TOP) || (m_ls->mapData[1][collisionPoint[1]].tileIndex == TILE_WOOD_FRAME_TOP))
        {
            m_blockedDirs[2] = 1;
        }
    }
    ...
    collisionPoint[0] = ((((m_ls->playerPos.y>>8) + 32 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_LEFT)) >> 5);
    	collisionPoint[1] = ((((m_ls->playerPos.y>>8) + 32 )>>4)*80) + (((m_ls->playerPos.x>>8) + (PLAYER_BBOX_RIGHT)) >> 5);
...
void CPlayer::ProcessLayer2Collisions()
{
    ...
    lastCollision[0] = -1; lastCollision[1] = -1;
                    ...
                    if (m_ls->seconds < 30)
                   {
                         ...
                         (sortedCollisions[i][1] << 5) + 23) ) )
                        {
							...
							if (objIndex > -1)
							{
								...
								if (m_ls->seconds > 30) {m_ls->seconds = 30;}
                ...
                case TILE_QUIVER: //Quiver
                    ...
                    if (m_ls->arrows < 12)
                    {
						...
						if (objIndex > -1)
						{
							...
							if (m_ls->arrows > 12) {m_ls->arrows = 12;}
