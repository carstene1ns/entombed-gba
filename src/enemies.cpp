...
CEnemy::CEnemy(T_LEVELSTATE* ls, FIXED _x, FIXED _y, FIXED _dx, FIXED _dy, int _type, int _xMin, int _xMax, int _hitPoints, int _fireRate,  int _obj_index)
{
	x = _x<<8; y = _y<<8; dx = _dx; dy = _dy; type = _type; xMin = _xMin; xMax = _xMax; obj_index = _obj_index;
	active = true;
	...
	switch (type)
	{
		...
		case TILE_SNAKE: //Snake
			...
			obj_set_attr(obj,
			 ...
			 ATTR2_PALBANK(0) |
			 ATTR2_PRIO(2) |
			 ATTR2_ID( (dx<0) ? SPRTILES_ENEMIES : SPRTILES_ENEMIES + (16*2))); //Facing left or right depending on speed.
		break;
		...
		case TILE_SPHINX: //Sphinx
			...
			obj_set_attr(obj,
			 ...
			 ATTR2_PALBANK(0) |
			 ATTR2_PRIO(2) |
			 ATTR2_ID((dx<0) ? SPRTILES_ENEMIES + (16*8): SPRTILES_ENEMIES + (16*10)));	//Facing left or right depending on speed.
		break;
	...
	if (isDying == false)
	{
		...
		else
		{
			...
			else
			{
				...
				BFN_SET(obj->attr2, SPRTILES_ENEMIES + (16*8) + ((dx > 0) ? 32 : 0) + ((animFrame>>8) * 16), ATTR2_ID);
			}
